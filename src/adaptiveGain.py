...
def load_stft_data(signal_name, base_folder='matrix', file_name='stft_data.npz'):
    ...
    Parameters:
    - signal_name (str): Name of the input signal (without extension).
...
def save_enhanced_stft_data(enhanced_stft, fs, signal_name, base_folder='matrix'):
    ...
    """
    sub_folder = os.path.join(base_folder, signal_name)
    output_path = os.path.join(sub_folder, 'adaptiveGain.npz')
...
if __name__ == "__main__":
    ...
    # Inverse STFT for debugging
    nperseg = 2 * (stft_matrix.shape[0] - 1)
    _, enhanced_time = istft(enhanced_stft, fs=fs, window='hann', nperseg=nperseg, noverlap=nperseg//2)
    wavfile.write(os.path.join('noisy_train', 'enhanced.wav'), fs, (enhanced_time * 32767).astype(np.int16))
    print(f"Enhanced WAV saved for PESQ check")

    # Noise PSD check
    print(f"Noise PSD mean/max: {np.mean(noise_psd):.2f}, {np.max(noise_psd):.2f}")

    # After noise_psd
    print(f"Mean energy: {np.mean(energies):.4f}, Threshold: {threshold:.4f}, Non-speech frames: {np.sum(non_speech_mask)}")
    if noise_frames.size == 0:
        print("No noise frames detected! Using fallback.")
        noise_psd = np.mean(magnitude**2, axis=1, keepdims=True) * 0.1

    # After istft
    from pesq import pesq
    _, clean = wavfile.read(os.path.join('clean_train', signal_name + '.wav'))
    clean = clean.astype(np.float64) / 32768.0
    min_length = min(len(clean), len(enhanced_time))
    score = pesq(fs, clean[:min_length], enhanced_time[:min_length], 'wb')
    print(f"PESQ (enhanced vs clean): {score:.3f}")